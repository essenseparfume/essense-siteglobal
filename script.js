...
const SOCIAL={telegram:"#",tiktok:"#"}; // Замінити # на свої посилання пізніше.
...
$("#orderForm").addEventListener("submit",async e=>{
 ...
 try{await fetch(TG_WEBAPP,{method:"POST",body:data});alert("Дякуємо! Замовлення прийнято. Ми зв'яжемося з вами.");cart=[];save();$("#checkoutModal").classList.remove("open");$("#cartDrawer").classList.remove("open");$("#orderForm").reset()}
 catch(err){alert("Не вдалося відправити замовлення. Спробуйте ще раз.")}
});
function setSocial(){
 ["telegramLink","telegramLink2"].forEach(id=>$("#"+id).href=SOCIAL.telegram);
 ["tiktokLink","tiktokLink2"].forEach(id=>$("#"+id).href=SOCIAL.tiktok);
}
